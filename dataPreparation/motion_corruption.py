...
def write_image(folder, image, imageName):
    ...
    sitk.WriteImage(corrupted_img, folder+"corrupted_images/"+"corrupted_"+imageName)
    
def generate_motion_trajectory(imagesize, x_shift=10, y_shift=10, z_shift=5, rotation=np.pi/90 ):
    ...
    trajectory[:,:,1] = trajectory[:,:,1]+1 # Adding 1 to avoid creation of invalid transformation 
    for slice in range(imagesize[2]):
        num = list(range(0,int(imagesize[0]/2-10))) + list(range(int(imagesize[0]/2+10),imagesize[0])) 
        motion_events = np.random.randint(100,high=200)
        ...
        for row in k_space_lines:
           shift = np.array([np.random.normal(loc=0,scale=x_shift),np.random.normal(loc=0,scale=y_shift),z_shift]) #Draw random samples from a normal (Gaussian) distribution.
           rot = np.random.normal(loc=0, scale=rotation) #Draw random samples from a normal (Gaussian) distribution.
           axes =np.array([0,0,1])
